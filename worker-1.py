...
def publish_data(record_id,record):
    print("data from publisher for worker 1")
...
def getbyname(name):
    candidate_data=[] # list to store if there are multiple records
    ...
    if(len(candidate_data)>0):
        return {
            'error': False,
            'result from worker 1': candidate_data
        }
    return {
        'error': True,
        'result from worker 1': "There is no data with the given name!"
    }
